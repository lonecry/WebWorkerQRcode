...
self.onmessage = function (e){
    ...
    qr.ecclevel = e.data.ecclevel;//输出类型
    var code = qr.init();//初始化
    console.log('成功发送到主线程');
    self.postMessage(code);
    function SunWenQR( customize ) {

            customize = typeof(customize) === "object" ? customize : {};//customize是一个空对象

            /*++++++++++++++++++++++++++++++Public+++++++++++++++++++++++++++++++++++++*/

            //如果浏览器不支持canvas,默认转换成table模式渲染
            ...
            // 控制选项 - 宽度是基于二维码版本的。后面四个是用于生成table的
            var ecclevel;
            var version;
            var width;
            var neccblk1;
            ...
            this.genframe = function(instring) {
                ...
                do {
                    ...
                    if (t <= k)
                        /*console.log("纠错等级"+ecclevel);
                        console.log('数据码字  '+datablkw);
                        console.log('纠错码字  '+eccblkwid);
                        console.log('k = '+k+' = '+datablkw+'x('+neccblk1+'+'+neccblk2+')+'+neccblk2+'-'+3+'+('+version+'<= 9'+')'+'\n\n');
                        */
                        break;//break是结束整个循环体
                } while (version < 40);
                // 根据版本号，计算出二维码的大小，场和宽个几个“像素点”
                width = 17 + 4 * version;
                console.log('计算出版本'+version);
